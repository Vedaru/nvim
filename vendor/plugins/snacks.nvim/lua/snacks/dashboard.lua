...
function D:init()
  ...
  vim.keymap.set("n", "q", "<cmd>qa<cr>", { silent = true, buffer = self.buf })
  ...
  vim.api.nvim_create_autocmd("WinEnter", {
    ...
    callback = function(ev)
      ...
      -- 重新吸附光标：从 Lazy 等浮窗 q 退出回到 dashboard 时，dashboard
      -- 窗口本身仍有效，原条件 (not win_is_valid) 不会触发 update，导致
      -- CursorMoved 吸附 autocmd 失效。这里在所有回到 dashboard buffer 的
      -- 场景下都重新 update 一次；schedule 等浮窗完全关闭后再执行。
